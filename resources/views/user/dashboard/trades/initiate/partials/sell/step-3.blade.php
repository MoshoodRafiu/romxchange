...
@else
    @if($trade->is_special == 1)
        @if(($trade->seller_transaction_stage == 2 && $trade->buyer_transaction_stage == 2) || ($trade->seller_transaction_stage == 2 && $trade->buyer_transaction_stage == 3))
        ...
        @elseif($trade->seller_transaction_stage == 2 && $trade->buyer_transaction_stage == 4)
            <div class="text-center" id="transaction-message">
                <strong class="text-success" id="info-3-text" style="font-size: 23px">Payment Made, Confirm Payment and Proceed</strong>
                <img width="100px" id="info-3-img" src="{{ asset('assets/img/proceed.gif') }}" alt="waiting">
            </div>
        @endif
    @else
        ...
        @elseif($trade->seller_transaction_stage == 2 && $trade->buyer_transaction_stage == 3)
            <div class="text-center" id="transaction-message">
                <strong class="text-success" id="info-3-text" style="font-size: 23px">Payment Made, Confirm Payment and Proceed</strong>
                <img width="100px" id="info-3-img" src="{{ asset('assets/img/proceed.gif') }}" alt="waiting">
            </div>
...
<div class="mx-auto text-center" id="trade-cancel">
    ...
    @else
        @if($trade->is_special == 1)
            ...
            @endif
        @else
            @if(($trade->buyer_transaction_stage == 2 && $trade->seller_transaction_stage == 2) || ($trade->buyer_transaction_stage == 3 && $trade->seller_transaction_stage == 2))
                @if($trade->is_dispute == 1)
                    <button type="button" disabled class="btn btn-info p-2">Dispute Trade</button>
                @else
                    <a href="{{ route('trade.dispute', $trade) }}" class="btn btn-info p-2">Dispute Trade</a>
